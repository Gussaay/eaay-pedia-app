// AddMainCategoryActivity -> AddNewMcqsActivity -> AddNewQuizActivity,
// plus edit and delete for every level.
//
// Deleting a container (category or book) does not delete what sits under it:
// the app matches books to categories, and quizzes to books, by the "source"
// string, so a cascade would have to guess. The dialogs say plainly what is
// left behind, and the same applies when a "source" is renamed.
import { useMemo, useState } from 'react';
import { useNavigate, useParams, useSearchParams } from 'react-router-dom';
import { Pencil, Plus, Trash2 } from 'lucide-react';
import { useAuth } from '../../hooks/useAuth';
import { useList } from '../../hooks/useData';
import { newKey, num, pushTo, removeAt, updateAt } from '../../lib/rtdb';
import { SUPER_ADMIN_EMAIL } from '../../config';
import ImageField from '../../components/ImageField';
import DangerConfirm from '../../components/DangerConfirm';
import { AppBar, Button, Card, Empty, Input, ListCard, Modal, Page, SkeletonList, Textarea, Toggle, useToast } from '../../components/ui';

const DB_KEY_RE = /[.#$[\]/]/;
const plural = (n, word) => `${n} ${word}${n === 1 ? '' : 's'}`;

/** Pencil + bin buttons shown on the right of a list row. */
function RowActions({ onEdit, onDelete }) {
  const stop = (fn) => (e) => {
    e.stopPropagation();
    fn();
  };
  return (
    <div className="flex items-center gap-1 shrink-0">
      <button onClick={stop(onEdit)} aria-label="Edit" className="p-2 rounded-lg text-slate-400 hover:text-brand-700 hover:bg-brand-50">
        <Pencil size={18} />
      </button>
      <button onClick={stop(onDelete)} aria-label="Delete" className="p-2 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50">
        <Trash2 size={18} />
      </button>
    </div>
  );
}

function AddButton({ onClick, children }) {
  return (
    <Button className="w-full" onClick={onClick}>
      <Plus size={18} /> {children}
    </Button>
  );
}

// ---------------------------------------------------------------------------
export function Categories() {
  const { user } = useAuth();
  const navigate = useNavigate();
  const toast = useToast();
  const list = useList('main_category');
  const books = useList('mcqs');
  const [form, setForm] = useState(null); // { _key?, title, source, img, publish }
  const [confirm, setConfirm] = useState(null);
  const [busy, setBusy] = useState(false);
  const canAdd = user.email === SUPER_ADMIN_EMAIL;

  const booksUnder = (source) => books.data.filter((b) => b.main_category === source).length;

  const save = async () => {
    if (!form.source.trim()) return toast('Enter source', 'error');
    if (!form.title.trim()) return toast('Enter title', 'error');
    if (DB_KEY_RE.test(form.source)) return toast('Source cannot contain . # $ [ ] /', 'error');
    setBusy(true);
    try {
      const record = {
        title: form.title.trim(),
        source: form.source.trim(),
        type: 'quiz',
        publish: form.publish ? 'true' : 'false',
        img: form.img,
      };
      if (form._key) await updateAt(`main_category/${form._key}`, record);
      else await pushTo('main_category', record);
      toast(form._key ? 'Category saved' : 'Category added', 'success');
      setForm(null);
      list.reload();
    } catch (e) {
      toast(e.message, 'error');
    } finally {
      setBusy(false);
    }
  };

  const doDelete = async () => {
    setBusy(true);
    try {
      await removeAt(`main_category/${confirm._key}`);
      toast('Category deleted', 'success');
      setConfirm(null);
      list.reload();
    } catch (e) {
      toast(e.message, 'error');
    } finally {
      setBusy(false);
    }
  };

  return (
    <div className="min-h-screen">
      <AppBar title="Categories" subtitle="Tap a category to open its books" />
      <Page className="space-y-3">
        {canAdd ? (
          <AddButton onClick={() => setForm({ title: '', source: '', img: '', publish: true })}>Add new category</AddButton>
        ) : (
          <p className="text-sm text-slate-500">Only the main admin can add new sources.</p>
        )}
        {list.loading ? (
          <SkeletonList />
        ) : (
          list.data.map((c) => (
            <ListCard
              key={c._key}
              img={c.img}
              title={c.title}
              subtitle={`source: ${c.source} · ${plural(booksUnder(c.source), 'book')}${String(c.publish) === 'true' ? '' : ' · hidden'}`}
              onClick={() => navigate(`/admin/category/${encodeURIComponent(c.source)}?title=${encodeURIComponent(c.title || '')}`)}
              right={
                <RowActions
                  onEdit={() => setForm({ _key: c._key, title: c.title || '', source: c.source || '', img: c.img || '', publish: String(c.publish) === 'true' })}
                  onDelete={() => setConfirm(c)}
                />
              }
            />
          ))
        )}
      </Page>

      <Modal
        open={!!form}
        onClose={() => setForm(null)}
        title={form?._key ? 'Edit category' : 'New category'}
        footer={<Button onClick={save} loading={busy}>{form?._key ? 'Save' : 'Upload'}</Button>}
      >
        {form && (
          <>
            <Input label="Title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
            <Input
              label="Source (unique id, e.g. nelson)"
              hint={form._key ? 'Books are linked to this category by source — changing it hides the books already under it.' : undefined}
              value={form.source}
              onChange={(e) => setForm({ ...form, source: e.target.value })}
            />
            <ImageField folder="source" value={form.img} onChange={(img) => setForm({ ...form, img })} />
            <Toggle label="Published (visible to users)" checked={form.publish} onChange={(v) => setForm({ ...form, publish: v })} />
          </>
        )}
      </Modal>

      <DangerConfirm
        open={!!confirm}
        busy={busy}
        title="Delete category?"
        message={`"${confirm?.title}" will be removed from the home screen for every user.`}
        impact={['The category disappears from the app immediately']}
        keeps={
          booksUnder(confirm?.source) > 0
            ? [`Its ${plural(booksUnder(confirm?.source), 'book')} and their quizzes stay in the database but become unreachable — open the category and delete them first if you want them gone`]
            : []
        }
        onCancel={() => setConfirm(null)}
        onConfirm={doDelete}
      />
    </div>
  );
}

// ---------------------------------------------------------------------------
export function CategoryBooks() {
  const { source } = useParams();
  const [params] = useSearchParams();
  const navigate = useNavigate();
  const toast = useToast();
  const list = useList('mcqs', { filter: (b) => b.main_category === source });
  const quizzes = useList('allquiz');
  const [form, setForm] = useState(null);
  const [confirm, setConfirm] = useState(null);
  const [busy, setBusy] = useState(false);

  const quizzesUnder = (b) =>
    quizzes.data.filter((q) =>
      String(b.source || '').includes('smsb')
        ? String(q.source || '').includes(b.source) && q.type === (b.sub_category || '')
        : q.source === b.source,
    ).length;

  const save = async () => {
    if (!form.source.trim()) return toast('Enter source', 'error');
    if (!form.title.trim()) return toast('Enter title', 'error');
    setBusy(true);
    try {
      const record = {
        title: form.title.trim(),
        source: form.source.trim(),
        sub_category: form.sub_category.trim(),
        img: form.img,
        // Needed so the book shows under its category (McqsMainActivity filter).
        main_category: source,
        type: 'quiz',
      };
      if (form._key) await updateAt(`mcqs/${form._key}`, record);
      else await pushTo('mcqs', record);
      toast(form._key ? 'Book saved' : 'Book added', 'success');
      setForm(null);
      list.reload();
    } catch (e) {
      toast(e.message, 'error');
    } finally {
      setBusy(false);
    }
  };

  const doDelete = async () => {
    setBusy(true);
    try {
      await removeAt(`mcqs/${confirm._key}`);
      toast('Book deleted', 'success');
      setConfirm(null);
      list.reload();
    } catch (e) {
      toast(e.message, 'error');
    } finally {
      setBusy(false);
    }
  };

  return (
    <div className="min-h-screen">
      <AppBar title={params.get('title') || source} subtitle="Books — tap one to open its quizzes" />
      <Page className="space-y-3">
        <AddButton onClick={() => setForm({ title: '', source, sub_category: '', img: '' })}>Add new book</AddButton>
        {list.loading ? (
          <SkeletonList />
        ) : list.data.length === 0 ? (
          <Empty title="No books in this category yet" />
        ) : (
          list.data.map((b) => (
            <ListCard
              key={b._key}
              img={b.img}
              title={b.title}
              subtitle={`source: ${b.source}${b.sub_category ? ` · type: ${b.sub_category}` : ''} · ${plural(quizzesUnder(b), 'quiz').replace('quizs', 'quizzes')}`}
              onClick={() =>
                navigate(
                  `/admin/book/${encodeURIComponent(b.source)}?sub=${encodeURIComponent(b.sub_category || '')}&title=${encodeURIComponent(b.title || '')}`,
                )
              }
              right={
                <RowActions
                  onEdit={() => setForm({ _key: b._key, title: b.title || '', source: b.source || '', sub_category: b.sub_category || '', img: b.img || '' })}
                  onDelete={() => setConfirm(b)}
                />
              }
            />
          ))
        )}
      </Page>

      <Modal
        open={!!form}
        onClose={() => setForm(null)}
        title={form?._key ? 'Edit book' : 'New book'}
        footer={<Button onClick={save} loading={busy}>{form?._key ? 'Save' : 'Upload'}</Button>}
      >
        {form && (
          <>
            <Input label="Title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
            <Input
              label="Source"
              hint={form._key ? 'Quizzes are matched to this book by source — changing it hides the quizzes already under it.' : 'Quizzes are matched to this book by source.'}
              value={form.source}
              onChange={(e) => setForm({ ...form, source: e.target.value })}
            />
            <Input label="Sub-category / type (optional)" hint="Used by collection books (smsb) to split parts." value={form.sub_category} onChange={(e) => setForm({ ...form, sub_category: e.target.value })} />
            <ImageField folder="source" value={form.img} onChange={(img) => setForm({ ...form, img })} />
          </>
        )}
      </Modal>

      <DangerConfirm
        open={!!confirm}
        busy={busy}
        title="Delete book?"
        message={`"${confirm?.title}" will be removed from this category for every user.`}
        impact={['The book disappears from the app immediately']}
        keeps={
          confirm && quizzesUnder(confirm) > 0
            ? [`Its ${quizzesUnder(confirm)} quiz(zes) and their questions stay in the database but become unreachable — open the book and delete them first if you want them gone`]
            : []
        }
        onCancel={() => setConfirm(null)}
        onConfirm={doDelete}
      />
    </div>
  );
}

// ---------------------------------------------------------------------------
export function BookQuizzes() {
  const { source } = useParams();
  const [params] = useSearchParams();
  const sub = params.get('sub') || '';
  const navigate = useNavigate();
  const toast = useToast();
  const { user } = useAuth();
  const isCollection = source.includes('smsb');
  const list = useList('allquiz', {
    filter: (q) => (isCollection ? String(q.source || '').includes(source) && q.type === sub : q.source === source),
    sort: (a, b) => (a._key < b._key ? 1 : -1),
  });
  const [form, setForm] = useState(null);
  const [confirm, setConfirm] = useState(null);
  const [busy, setBusy] = useState(false);

  const open = () =>
    setForm({ title: '', des: '', img: '', uploader: user.email === SUPER_ADMIN_EMAIL ? 'Dr. Qusay Mohamed' : '', preview: true, month: '', year: '' });

  const save = async () => {
    if (!form.title.trim()) return toast('Enter title', 'error');
    if (!form.des.trim()) return toast('Enter description', 'error');
    if (!form.img) return toast('Please upload image', 'error');
    if (source === 'smsb' && (!form.month.trim() || !form.year.trim())) return toast('Enter month and year of the exam', 'error');
    setBusy(true);
    try {
      const key = newKey('allquiz');
      const record = {
        title: form.title.trim(),
        des: form.des.trim(),
        img: form.img,
        key,
        uploader: form.uploader.trim(),
        source,
        preview: form.preview ? 'true' : 'false',
        number: '0',
        type: sub,
      };
      if (source === 'smsb') Object.assign(record, { month: form.month.trim(), year: form.year.trim() });
      await pushTo('allquiz', record);
      toast('Uploaded', 'success');
      setForm(null);
      list.reload();
    } catch (e) {
      toast(e.message, 'error');
    } finally {
      setBusy(false);
    }
  };

  const doDelete = async () => {
    setBusy(true);
    try {
      await removeAt(`allquiz/${confirm._key}`);
      toast('Quiz deleted', 'success');
      setConfirm(null);
      list.reload();
    } catch (e) {
      toast(e.message, 'error');
    } finally {
      setBusy(false);
    }
  };

  return (
    <div className="min-h-screen">
      <AppBar title={params.get('title') || source} subtitle="Quizzes — tap one to edit it or add questions" />
      <Page className="space-y-3">
        <AddButton onClick={open}>Add new quiz</AddButton>
        {list.loading ? (
          <SkeletonList />
        ) : list.data.length === 0 ? (
          <Empty title="No quizzes in this book yet" />
        ) : (
          list.data.map((q) => (
            <ListCard
              key={q._key}
              img={q.img}
              title={q.title}
              subtitle={`${num(q.number)} questions${String(q.preview) === 'true' ? ' · preview' : ''}`}
              onClick={() => navigate(`/admin/quiz/${encodeURIComponent(q._key)}`)}
              right={
                <RowActions
                  onEdit={() => navigate(`/admin/quiz/${encodeURIComponent(q._key)}`)}
                  onDelete={() => setConfirm(q)}
                />
              }
            />
          ))
        )}
      </Page>

      <Modal open={!!form} onClose={() => setForm(null)} title="New quiz" footer={<Button onClick={save} loading={busy}>Upload</Button>}>
        {form && (
          <Card className="p-0 border-0 shadow-none">
            <Input label="Title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
            <Textarea label="Description" value={form.des} onChange={(e) => setForm({ ...form, des: e.target.value })} />
            <Input label="Uploaded by" value={form.uploader} onChange={(e) => setForm({ ...form, uploader: e.target.value })} />
            {source === 'smsb' && (
              <div className="grid grid-cols-2 gap-3">
                <Input label="Month" value={form.month} onChange={(e) => setForm({ ...form, month: e.target.value })} />
                <Input label="Year" value={form.year} onChange={(e) => setForm({ ...form, year: e.target.value })} />
              </div>
            )}
            <ImageField label="Cover image" folder="allquiz_sorage" value={form.img} onChange={(img) => setForm({ ...form, img })} />
            <Toggle label="Save as preview (not visible to users yet)" checked={form.preview} onChange={(v) => setForm({ ...form, preview: v })} />
          </Card>
        )}
      </Modal>

      <DangerConfirm
        open={!!confirm}
        busy={busy}
        title="Delete quiz?"
        message={`"${confirm?.title}" will disappear from the app, along with everyone's ability to play it.`}
        impact={['The quiz is removed from the book immediately']}
        keeps={[
          `Its ${num(confirm?.number)} questions stay in the database — open the quiz and use "Delete all questions" first if you want them removed`,
          'Scores and leaderboard entries users already earned are kept',
        ]}
        onCancel={() => setConfirm(null)}
        onConfirm={doDelete}
      />
    </div>
  );
}
